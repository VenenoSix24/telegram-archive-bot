...
- **归档**：浏览归档消息与缩略图；按关键词（搜索原文与渲染文本）、Tag（多选取交集）、评级、媒体类型、目标频道筛选；详情页直接改评级、增删 Tag，保存后写库 → 重渲染 → 同步 Telegram 消息与置顶索引
- **Tags**：Tag 统计视图
- **设置**：配置编辑器（保存自动备份并提示重启生效）、配置 / 数据库的备份、下载、恢复、导入，以及重置数据库
- **双主题**：`minimal`（简约风，默认）与 `collection`（素材志）两套主题，各有可选配色方案；明暗模式支持浅色 / 深色 / 跟随系统

...
const exportBusy = ref<'csv' | 'json' | null>(null)

/* 导出归档：GET 下载由 Content-Disposition 触发，Cookie 会话自动携带 */
function exportArchive(fmt: 'csv' | 'json') {
  if (exportBusy.value) return
  exportBusy.value = fmt
  const url = `/api/v1/ops/export?format=${fmt}`
  ...
  a.click()
  a.remove()
  // 浏览器没有下载完成的回调，按钮态只做短暂防抖
  window.setTimeout(() => (exportBusy.value = null), 1500)
}
